...
export default function Portfolio(props) {
  return (
    <section className='flex flex-col items-center gap-8'>
      ...
      <section key={project.id} className='text-stone-900 bg-yellow-600 flex flex-col gap-2 items-center w-11/12 p-4 rounded-md max-w-lg shadow-rounded'>
        ...
        <div className='flex flex-col items-center'>
          ...
          <div className='bg-sky-300 p-2 flex gap-2 rounded-xl border-y-2 border-stone-900'>
            {project.techIcons.map(icon => (
              <i className={`${icon} bg-stone-200 p-2 rounded`}></i>
            ))}
          </div>
        </div>
